...
$stmt_o->close();
        ...
        define( 'PAGINA_RETORNO', '../publico/production/dist_nuevaCompra.php?accion=mensajePagado' );
        header( 'Location: '.PAGINA_RETORNO );
        }  
...
$stmt_o -> close();
    ...
    if(isset($_GET['origen']) && $_GET['origen'] == 'simple'){
        define( 'PAGINA_RETORNO', '../publico/production/dist_nuevaCompra.php?accion=mensajeDeshacer' );
        header( 'Location: '.PAGINA_RETORNO );  
    }else{
...
$var = "dist";
    ...
    $precio2 = number_format($precio2,'2', ',','.');
        ...
        $precioBsVenta = round($resultado5 * $cantidadNueva, 2, PHP_ROUND_HALF_DOWN);    /// PRECIO TOTAL PARA EL PRODUCTO EN BOLIVARES
        $precioPesoVenta = round($resultado6 * $cantidadNueva, 2, PHP_ROUND_HALF_DOWN);  /// PRECIO TOTAL PARA EL PRODUCTO EN PESOS
    ...
    $valores='("'.$_SESSION['proveedor'].'","'.$_SESSION['factura'].'","'.$_SESSION['fechaFactura'].'","'.$_SESSION['statusFactura'].'","'.$codigo.'","'.$nombrepor.'","'.$precio.'","'.$cantidadNueva.'","'.$cantidadporprecio.'","'.$Porcentaje.'","'.$fdeCompra.'","'.$fdeUser.'","'.$SdeCompra.'","'.$ddeCompra.'","'.$mdeCompra.'","'.$precio2.'","'.$precioPesoVenta.'","'.$precioBsVenta.'","'.$moneda.'","'.$precioMonedaOrigen.'","'.$Tasas_pesos.'","'.$Tasas_dolares.'","'.$var.'"),';
...
if(isset($_GET['nuevoRegistro'])){
    ...
    echo $nombre."<br>";
    $insertar = "INSERT INTO compras (proveedor, factura, fechaFactura, status, cod, producto, precio, cantidad, CporPrecio, Porcentaje, fecha, user, semana, dia, mes, dolaresSubTotal, pesosSubTotal, bolivaresSubTotal, MonedaOrigen, MontoOrigen, TasaPeso, TasaDolar, tipo) VALUES $nombre";
    ...
    if ( $buscarAlumnos2->num_rows > 0 ) {
        while( $filaAlumnos2 = $buscarAlumnos2->fetch_assoc() ){
            ...
            $MonedaOrigen = $filaAlumnos2['MonedaOrigen'];
            $MontoOrigen = $filaAlumnos2['MontoOrigen'];
            $TasaPeso = $filaAlumnos2['TasaPeso'];
            $TasaDolar = $filaAlumnos2['TasaDolar'];
    ...
    $stmt_o = $conexion->prepare("UPDATE productos SET precio_compra='$precionuevo', cantidad_unidades='$CporPrecio', porcentaje='$PorcentajeNuevo', stock='$cantidadFinal',
    MonedaOrigen='$MonedaOrigen', MontoOrigen='$MontoOrigen', TasaPeso='$TasaPeso', TasaDolar='$TasaDolar' WHERE codigo='$cod'");
$stmt_o->execute();
$stmt_o -> close();
...
if ( $buscarAlumnos22->num_rows > 0 ) {
    ...
    define( 'PAGINA_RETORNO', '../publico/production/dist_nuevaCompra.php?accion=mensajeExito' );
    header( 'Location: '.PAGINA_RETORNO ); 
        }
...
////////////////////////////////////////// ESTO INSERTA LAS FACTURAS A LA BASE DE DATOS

define( 'PAGINA_RETORNO', '../publico/production/dist_nuevaCompra.php' );
        header( 'Location: '.PAGINA_RETORNO );
?>
...
try {
  ...
  $user_cond = "";
  $userCondO = "";
  if (isset($data['usuario']) && $data['usuario'] != 'todos' && $data['usuario'] != '') {
    $usuario = (int)$data['usuario'];
    $user_cond = " AND usuario  = $usuario";
    $userCondO = " AND o.usuario = $usuario";
  }
...
SELECT 
    ...
    FROM (
        ...
        WHERE o.bss_id = $bss_id 
          AND o.status IN (1,4) 
          AND o.cliente > '' 
          $extraCond $userCondO $periodCond 
        GROUP BY o.cliente 
...
";
  ...
  $sql = "
    ...
    FROM (
        ...
        WHERE o.bss_id = $bss_id 
          AND o.status IN (1,4) 
          $extraCond $userCondO $periodCondProductos
...
";
  ...
  $res = $conexion->query("SELECT dia, total_price FROM orden AS o WHERE semana = '$semana' AND status IN (1,4) AND bss_id = $bss_id $extraCond $user_cond");
  while ($row = $res->fetch_assoc()) {
    $dia = (int)$row['dia'];
  ...
  {
    global $hoy, $dia_ant, $semana, $semana_ant, $mes, $mes_ant, $diasSemana, $conexion, $user_cond, $userCondO;
...
");
  ...
  $resDespachos = $conexion->query("
    ...
    FROM orden_articulos oa
    INNER JOIN orden o ON oa.order_id = o.id
    WHERE o.bss_id = $bss_id $extraCond $userCondO 

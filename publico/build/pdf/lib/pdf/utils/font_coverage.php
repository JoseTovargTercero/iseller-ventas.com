...
if (!class_exists('TTFontFile_Analysis', false)) { require_once(_MPDF_PATH .'classes/ttfontsuni_analysis.php'); }
...
require_once('UnicodeRanges.php'); 
//==============================================================
  ...
  foreach ($tempfontdata AS $fname => $v) {
	...
	foreach($unicode_ranges AS $urk => $ur) {
	   if ($urk >= ($urgp*$ningroup) && $urk < (($urgp+1)*$ningroup)) {
		...
		$html .= '<td style="background-color: #b1b1b1;"></td>'; 
	    }
	    else {
		$rangekey = $urk;
		$range = $ur['range'];
		...
		else { $html .= '<td style="background-color: #b1b1b1;"></td>'; }
	    }
	   }
	}

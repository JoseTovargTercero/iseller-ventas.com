...
function _fix_borderStr($bd) {
	...
	if ( count($prop) == 1 ) { 
		...
		// Change #b1b1b1 1px solid to 1px solid #b1b1b1 (proper)
		if (substr($prop[0],0,1) == '#') { $c = $prop[0]; $w = $prop[1]; $s = $prop[2]; }
		// Change solid #b1b1b1 1px to 1px solid #b1b1b1 (proper)
		else if (substr($prop[0],1,1) == '#') { $s = $prop[0]; $c = $prop[1]; $w = $prop[2]; }
		// Change solid 1px #b1b1b1 to 1px solid #b1b1b1 (proper)
  ...
  foreach(array('TOP','RIGHT','BOTTOM','LEFT') AS $side) {
	...
	if (isset($a['BORDER-'.$side.'-'.$el])) {	// e.g. $b['BORDER-TOP-STYLE']
		...
		if ($el=='STYLE') {
			...
			else { $b['BORDER-'.$side] = '0px '.$s.' #b1b1b1'; }
		}
		else if ($el=='WIDTH') {
			if ($p) { $b['BORDER-'.$side] = preg_replace('/(\S+)\s+(\S+)\s+(\S+)/', $s.' \\2 \\3', $p); }
			else { $b['BORDER-'.$side] = $s.' none #b1b1b1'; }
		}
		else if ($el=='COLOR') {

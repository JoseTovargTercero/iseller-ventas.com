...
if(isset($_GET['anterior'])){
    ...
    if($_GET['anterior'] <= 1){
     $mesConsulta = date('Y')."-01";    
    }else{ 
     $mesConsulta = date('Y')."-".$_GET['anterior'];
    }
    
    $mesAnterior = $_GET['anterior'] - 1;
...
$elmesname = $mesAnterior + 1;
$elmesname = "0".$elmesname;
...
if ($buscarAlumnosaaaaaaa->num_rows > 0) {
    ...
    $query00 = "SELECT * FROM orden WHERE fecha='$mesConsulta' AND status='1'";
    ...
    $query000 = "SELECT * FROM orden WHERE fecha='$mesConsulta' AND status='2'";
    ...
    $query0000 = "SELECT * FROM orden WHERE fecha='$mesConsulta' AND status!='5' AND status!='5.2'";
    ...
    $query000000 = "SELECT * FROM orden WHERE fecha='$mesConsulta' AND status='4'";
...
<p style="text-align:center"><span style="font-size:15px">'.$nameempresa.' - AVANCE DEL MES</span><br>
...
<br>
 ...
 <p style="text-align:center"><span style="font-size:15px">INGRESOS</span><br></p>
...
$html.= '
 ...
 <p style="text-align:center"><span style="font-size:15px">MOVIMIENTOS DEL MES</span><br></p>
 ...
 ';
            ...
            $query77 = "SELECT * FROM orden WHERE fecha='$mesConsulta' AND status='1' OR fecha='$mesConsulta' AND status='4' OR fecha='$mesConsulta' AND status='2'";
...
<p>(V) Venta al detal. (M) Venta al mayor. (C) Creditos</p>  
 ...
 <p style="text-align:center"><span style="font-size:15px">AGREGADOS AL STOCK ESTE MES</span><br></p>
...
<thead>
     ...
     $query6 = "SELECT * FROM compras WHERE mes='$mesConsulta' AND tipo!='dist'  ORDER BY id DESC LIMIT 150";
                            $buscarAlumnos6 = $conexion->query( $query6 );
                            if ( $buscarAlumnos6->num_rows > 0 ) {
                                $contador2 = 1;
                                while( $row6 = $buscarAlumnos6->fetch_assoc() ) {
    ...
    if($contador <= $deshacerCompra){
        ...
        $deshacer = '';
                ...
                if($row6["status"] == 2){
                  $statusF =  "<span style='color: orangered'>Pendiente</span>";
...
<a href="reporteMes.php?anterior='.$mesAnterior.'"> -- Mes Anterior -- </a><br>


REPORTE "AVANCE MESUAL" DE "MI TIENDA".</footer>                                        
';

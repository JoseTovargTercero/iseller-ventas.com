...
var Graph = Graph || (function($) {
	...
	Private.defaults = function() {
		return {
			//default options
			x: ['Lunes', 'Marte', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo'],
			y: 10,
			attachTo: 'body',
			points: [0]
		};
	...
	Private.basics = function(height, width, graphHeight, graphWidth) {
		...
		return {
			...
			padding: 0, //keep labels from touching edges
			//single points
	...
	Private.design = function(obj) {
		...
		var colors = obj.colors || ['#d9534f', '#428bca', '#1caf9a', '#f0ad4e', '#9a4ef0', '#f0c44e', '#ad4ef0', '#f09a4e', '#656866', '#b1b1b1'];
...
var GraphLinear = GraphLinear || (function($) {
		...
		var pointHandle = function(action) {
			...
			$('svg line[id^="' + id + '"], svg path[id^="' + id + '"]').each(function() {
				if ($(this).attr('id').split('-')[1] === num) {
					...
					if (action === 'add') {
						$(this).css('stroke-width', obj.lineWidth || 4);
						$(this).css('opacity', 1);
						$(tooltip).show();
						$(tooltipRect).show();
					} else {
	...
	GraphLinear.prototype.animateLines = function() {
		//much thanks to http://jakearchibald.com/2014/animated-line-drawing-svg/ !
		var self = this.obj;
	...
	GraphPie.prototype.init = function(thing) {
		if (!this.obj.multipleDataSets) {
			...
			for (var i = 0, len = self.numPoints; i < len; ++i) {
				...
				howMuchOfPieInRadians = howMuchOfPie * 2 * Math.PI; //2pi = 460 deg

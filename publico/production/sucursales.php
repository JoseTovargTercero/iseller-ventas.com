...
<head>
  ...
  <style>
    ...
    .btn-edit-icon ion-icon { font-size: 16px; }

    #section_edit { max-width: 560px; margin: 0 auto; }
    ...
    .file-input-btn ion-icon { font-size: 16px; }
    .file-input-name {
      font-size: 12px;
      color: var(--dash-text-muted);
      overflow: hidden;
    ...
    .modal-sucursal .close:hover { opacity: 1; }
    .modal-sucursal .modal-body { padding: 20px 22px; }
    .modal-sucursal .modal-footer {
      border-top: 1px solid var(--dash-border);
      padding: 14px 22px;
    }
    .modal-sucursal select.form-control { appearance: auto; }
            ...
            <form id="sucursal-form" novalidate>
              ...
              <div class="mb-3">
                ...
                <datalist id="tipos-comercio">
                  <option value="Panadería"><option value="Carnicería"><option value="Heladería">
                  <option value="Frigorífico"><option value="Supermercado"><option value="Verdulería">
                  <option value="Pescadería"><option value="Tienda de Ropa"><option value="Joyería">
                  <option value="Farmacia"><option value="Ferretería"><option value="Papelería">
                  <option value="Librería"><option value="Tienda de Electrónica"><option value="Tienda de Mascotas">
                  <option value="Floristería"><option value="Barbería"><option value="Peluquería">
                  <option value="Restaurante"><option value="Cafetería"><option value="Tienda de Deportes">
                  <option value="Juguetería"><option value="Boutique"><option value="Auto Lavado">
                  <option value="Gimnasio"><option value="Tienda de Muebles"><option value="Centro de Estética">
                  <option value="Tienda de Telefonía"><option value="Otro">
                </datalist>
              </div>
              <div class="mb-3">
...
$(document).ready(function() {
  $('#btn-add-sucursal').on('click', function() { $addModal.modal('show'); });
  ...
  $('#sucursal-form').on('submit', function(e) {
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (!value) { Alerta.toast('error', 'Todos los campos son obligatorios.'); return; }
    }
    fetch('../../configurar/sucursales.php', { method: 'POST', body: formData })
      .then(res => res.text())
      .then(text => {
        let json;
...
function cargar_tabla() {
  fetch('../../configurar/sucursales_lista.php', {
    ...
    body: JSON.stringify({ table: 'sucursales', config: '_sucursales' })
  })
  .then(res => res.text())
  .then(text => {
    try {
...
new FormHandler({
  ...
  data_extra: [['id_editar', () => sucursal_editar]],
  onSuccess: (json) => { Alerta.toast('success', json.mensaje); cargar_tabla(); cancelarActualizacion(); },
  onError: (json) => { Alerta.toast('error', json.mensaje || 'Error desconocido.'); },
  onFail: (err) => { console.error('Fallo:', err); }
});
</script>
